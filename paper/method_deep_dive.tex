...
\begin{example}[Crash Scenario]
During a market crash (rare ``Crash'' regime):
\begin{itemize}
    \item MARL population: All agents are Bull specialists $\rightarrow$ \textbf{All fail}
    \item NichePopulation: 1-2 agents are Bear/Crash specialists $\rightarrow$ \textbf{They succeed}
\end{itemize}
The diverse population is more robust to regime changes.
\end{example}
